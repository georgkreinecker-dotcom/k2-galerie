...
const MD_PATH = '/notizen-georg/diverses/schreiben-an-august-softwarestand-k2.md'

export default function SchreibenAugustSoftwareNotizPage() {
  ...
  if (error) {
    body = (
      <p style={baseStyles.p}>
        Fehler: {error}. <Link to={PROJECT_ROUTES['k2-galerie'].notizen} style={baseStyles.a}>Zurück zu Notizen</Link>
      </p>
    )
  } else if (loading) {
    ...
    try {
      ...
      body = (
        <p style={baseStyles.p}>
          Fehler beim Anzeigen. <Link to={PROJECT_ROUTES['k2-galerie'].notizen} style={baseStyles.a}>Zurück zu Notizen</Link>
        </p>
      )
    }
  }
