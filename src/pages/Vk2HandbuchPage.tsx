/**
 * Eigenes Handbuch für die Vereinsplattform (VK2) – für Vereine, Vorstände und Mitglieder.
 * Quelle: public/vk2-handbuch/*.md
 */
...
const FALLBACK_ROUTE = PROJECT_ROUTES['vk2'].galerie

export default function Vk2HandbuchPage() {
  ...
  const handleZurueck = () => {
    ...
    backFallbackRef.current = setTimeout(() => {
      backFallbackRef.current = null
      if (window.location.pathname === HANDBUCH_PATH || window.location.pathname.endsWith('vk2-handbuch')) {
  ...
  const renderMarkdown = (text: string, chapterNumber?: number, onDocLink?: (path: string) => void) => {
    ...
    while (i < lines.length) {
      ...
      if (numLinkMatch) {
        ...
        const isInternalDoc = path.endsWith('.md') && !path.startsWith('http') && !path.startsWith('/')
        if (isInternalDoc && onDocLink) {
          ...
          const href = path.startsWith('http') ? path : path.startsWith('/') ? path : `${HANDBUCH_BASE}/${path}`
  ...
  const currentDocName = DOCUMENTS.find((d) => d.file === selectedDoc)?.name ?? 'Handbuch Vereinsplattform'
    ...
    <div className={printPreview ? 'benutzer-handbuch-wrapper benutzer-print-preview' : 'benutzer-handbuch-wrapper'} style={{ padding: '1.5rem 1rem', background: '#fffefb', minHeight: '100vh', color: '#1c1a18' }}>
      ...
      {!printPreview && (
        <header className="benutzer-no-print benutzer-header-wrap" style={{ marginBottom: '1.5rem', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '1.5rem', color: '#1c1a18', fontWeight: 600 }}>📖 Handbuch Vereinsplattform (VK2)</h1>
            <p style={{ margin: '0.25rem 0 0', fontSize: '0.9rem', color: '#6b7280' }}>Für Vereine – Mitgliederliste, Galerie optional, Kassa, Druck. Zum Lesen und Drucken.</p>
          </div>
      ...
      {printPreview ? (
        <>
            ...
            <>
              <div className="benutzer-deckblatt" aria-hidden>
                <div style={{ maxWidth: '28rem', margin: '0 auto' }}>
                  ...
                  <h1 style={{ margin: '1rem 0 0.5rem', fontSize: 'clamp(1.5rem, 4vw, 2rem)', fontWeight: 700, lineHeight: 1.25, color: '#fff' }}>
                    Handbuch Vereinsplattform (VK2)
                  </h1>
                  <div className="benutzer-deckblatt-fuss" style={{ marginTop: '3rem', paddingTop: '2rem', borderTop: '1px solid rgba(255,255,255,0.2)' }}>
                    <p style={{ margin: 0, fontSize: '1.25rem', fontWeight: 600 }}>Vereinsplattform</p>
                    <p style={{ margin: '0.25rem 0 0', fontSize: '1rem', color: 'rgba(255,255,255,0.9)' }}>Für Vereine – Mitgliederliste, Galerie optional, Kassa, Druck</p>
                  </div>
                </div>
              </div>
          ...
          <div className="seitenfuss" aria-hidden>
            <span className="seitenfuss-preview">VK2 Handbuch Vereinsplattform · (Seitenzahlen beim Drucken)</span>
          </div>
        </>
      ) : (
