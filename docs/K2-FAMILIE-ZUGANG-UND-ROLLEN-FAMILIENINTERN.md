...
- **Standard-Rolle** ohne gespeicherte Wahl: **Leser** (`loadFamilieRolleForTenant` in `src/utils/familieRollenStorage.ts`) – damit reicht ein Familienlink **nicht**, als Inhaber:in zu arbeiten, ohne die Rolle bewusst zu wählen. **Nach der Wahl in den Einstellungen** wird die Rolle auf diesem Browser in **localStorage** (plus Session-Spiegel) gehalten; alte reine Session-Werte werden beim ersten Laden nach local migriert.
