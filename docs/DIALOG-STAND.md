...
**Letzter Stand:** 15.03.26 – **Buchhaltung/Kassabuch: Summenzeile (Einnahmen, Ausgaben, Saldo)** – ganz einfach für jedermann/frau, gute Basis für den Start eines kleinen Unternehmens. BuchhaltungPage + KassabuchPage: drei Karten (Einnahmen grün, Ausgaben rot, Saldo blau/orange) für den gewählten Zeitraum; Berechnung aus filteredEntries (eingang + bank_an_kassa = Einnahmen, rest = Ausgaben). Tests grün, tsc ok. **(Davor 14.03.26)** – **(1) Eiserne Regel: ök2 ≠ APf; Handbuch bereinigt:** ök2 hat nichts mit APf (Programmiererplattform) zu tun; Vermischung strengstens verboten. Regel .cursor/rules/oek2-apf-niemals-vermischen.mdc (alwaysApply). Benutzerhandbuch 08-KASSA: alle Erwähnungen von Control Studio entfernt; Kassa nur noch über Admin beschrieben. **(2) Benutzerhandbuch: Pro++ Rechnungspflege + Lizenzstufen:** In 06-OEK2-DEMO-LIZENZ.md neue Sektion „Lizenzstufen im Überblick“ (Tabelle Basic/Pro/Pro+/Pro++/VK2 mit Preis und Inhalt). In 08-KASSA-VERKAUF-BELEGE.md: Abschnitt „Pro++: Vollständige Rechnungspflege nach Rechnungslegungsnorm“ (§ 11 UStG: fortlaufende Nummer, Pflichtangaben, USt-Aufschlüsselung); Hinweis zu Lizenzstufen am Kapitelanfang; „Kurz zusammengefasst“ um Pro++ Rechnungspflege ergänzt. **(2) Trial 2 Wochen + AGB:** Modell „2 Wochen gratis, dann zahlen oder Daten/Lizenz verfallen“ in AGB festgehalten (AGBPage §2): Nutzer gibt Daten ein; Versicherung unverzügliche Löschung bei Nichtzahlung; AGB-Bestätigung wie üblich (WillkommenPage Checkbox). OEK2-ANMELDUNG-LIZENZIERUNG-STAND Abschnitt 5 ergänzt. **(2) Flyer & Werbedokumente – keine K2-Daten in ök2 (eisernes Gesetz):** WerbeunterlagenPage: Galerie-URL/QR abhängig von Modus (ök2 → galerieOeffentlichVorschau). FlyerK2GaleriePage: Tenant aus URL/sessionStorage; bei oeffentlich nur MUSTER_TEXTE + loadStammdaten/loadEvents('oeffentlich'). ProspektGalerieeroeffnungPage: useProspektTenant(); bei oeffentlich Stammdaten/Events/QR/Impressum nur Muster; QR → ök2-Galerie-URL. PresseEinladungK2GaleriePage: usePresseTenant(); Stammdaten aus stammdatenStorage(tenant,'gallery'), Events loadEvents(tenant). **(2) Davor:** Rechnungen/Belege ök2, mök2 Positionierung. **Nächster Schritt:** Optional: statische Präsentationsmappen-MDs auf K2-Texte prüfen; Georg testen. (Commit folgt.)

**Was wir JETZT tun:** Buchhaltung/Kassabuch mit einfacher Summenzeile abgeschlossen – Einnahmen/Ausgaben/Saldo für den Zeitraum, für jeden verständlich.

**Nächster Schritt:** Georg testen (Buchhaltung, Kassabuch, Zeitraum wählen, Summen prüfen). Optional: Commit + Push ausführen falls noch nicht.
