...
**Letzter Stand:** 10.05.26 – **Kurzbezeichnungen L (Lizenzversion):** **LK2** = ök2, **LVK2** = VK2, **LFK2** = K2 Familie; **K2** = echte Galerie ohne L. **docs/KURZBEZEICHNUNGEN-L-LIZENZVERSION.md**, Verweise in STRUKTUR, HAUS-INDEX, 00-INDEX, Regel **struktur-handeln-quellen**, **`npm run briefing`** ergänzt. **qs:local** grün. **Commit:** `f66a0486` ✅ **main**
...
**Letzter Stand:** 09.05.26 – **Phishing / unsichere Links:** **`safeExternalUrl.ts`** (nur **http(s)**, kein **`javascript:`** / **`data:`** / **`//`**-Trick); **`openCheckoutOrPaymentUrl`** + Lizenz-Seiten **`blocked`**; Markdown-Schreibtisch/Notiz; **SmartPanel** „Diverses“; **VK2-Katalog** Galerie-Link; **Stammbaum**-Öffnen; **`window.open`** mit **`noopener,noreferrer`** (u. a. Vita-Blob, GitHub-Token, Second Mac, Buchhaltung-Druck); **`vercel.json`** **HSTS**; **`main.tsx`** iframe-Hinweis. Tests **`safeExternalUrl.test.ts`**, Checkout-Tests erweitert. **`npm run build`** grün. **Commit:** `20d60420` ✅ **main**

**Letzter Stand:** 09.05.26 – **CSP + Boot + Profilstufe B:** Stand-Logik in **`public/boot/boot-build-info.js`** (schreibt **`write-build-info.js`**); **`index.html`** ohne Inline-Script dafür; **`vercel.json`** **Content-Security-Policy** + Rewrite **`(?!boot/)`**; Guard **`vercel-config-guard.test.ts`**. Doku **`docs/SICHERHEIT-CSP-UND-SCHUTZSTUFE.md`** (intern **Stufe B** mit Ausnahmen); **`STAND-QR-SO-BLEIBT-ES`**, **`VERCEL-STAND-HANDY`**, **`KRITISCHE-ABLAEUFE`**, **`00-INDEX`**; Regeln **`stand-qr-niemals-zurueck`**, **`vercel-stand-funktioniert`**, **`daten-geraete-sync`**. **`qs:local`** grün. **Commit:** `712abaaf` ✅ **main**

**Letzter Stand:** 09.05.26 – **Sicherheit: Verantwortung & Außenwortlaut:** **`docs/SICHERHEIT-VERANTWORTUNG-AUSSENKOMMUNIKATION.md`** – Kurztext für Piloten/Zahlungsdienstleister; Nachweistabelle „was wir getan haben“; Querverweise **`DATENSICHERHEIT-ABSICHERUNG`**, **`SICHERHEIT-CSP`**, **`00-INDEX`**. **Commit:** `66fbe33d` ✅ **main**

**Letzter Stand:** 09.05.26 – **K2 Familie nur Plattform-Host:** **`App.tsx`** – **`/familie`**, **`/projects/k2-familie/*`**, Willkommen, **`/k2-familie-handbuch`** mit **`PlatformOnlyRoute`**; Regel + Doku + Lehren-Doku + Test **`platformOnlyRouteLicensee.test.ts`**. **qs:local** grün (ohne Build-Info-Commit). **Commit:** `92c1dce7` ✅ **main**
...
**Letzter Stand:** 01.05.26 – **mök2 Werbefahrplan – eigene Arbeitsseite (übersichtlich, K2-like)** – Route **`/projects/k2-galerie/werbefahrplan`**: `Mok2WerbefahrplanWorkplace` (Karten nach `vonISO`, Editor darunter); in **Marketing ök2** nur **`Mok2WerbefahrplanTeaser`** + Link „Werbefahrplan öffnen“; Speicher `k2-mok2-werbefahrplan` + **`MOK2_WERBEFAHRPLAN_UPDATED_EVENT`** (Teaser aktualisiert ohne Tab-Wechsel); Sidebar `mok2-werbefahrplan`, `Mok2Layout` Oberleiste. **`npm run build` grün.** **Commit:** `fa78b837` ✅ **main**

**Letzter Stand:** 01.05.26 – **mök2 Werbefahrplan** (vorher nur Sektion) – Sidebar + **`#mok2-werbefahrplan`**: Kampagnen Titel/Von-Bis/Aktivitäten; Default erste Kampagne; Utils + Tests `werbefahrplanMok2Storage`. **Commit:** `d92f8892` ✅ **main**
...
**Letzter Stand:** 19.04.26 – **Teilen: Fußtext/Tooltip kontextrichtig (kein Presse-„Verteiler“)** – **`PublicTeilenFixed`:** `SHARE_FOOTER_HINT` für `oeffentlich` / `vk2` / `familie`. **`GaleriePage`:** `galerieShareHintTitle` + `galerieShareHintFooter` (K2 / ök2 / VK2). **Commit:** `ef91a5bd` ✅ **main**
...
**Letzter Stand:** 30.03.26 – **„Server nicht konfiguriert“ (Testpilot):** API braucht **`PILOT_INVITE_SECRET`** (Vercel Environment + ggf. lokal `.env`). **PilotEinladungPage:** Bei genau dieser Meldung jetzt **konkrete** Hinweise Vercel vs. localhost statt nur „Link abgelaufen“. **Commit:** `ef91a5bd` ✅ **main** ✅ auf GitHub
...
**Letzter Stand:** 25.03.26 – **Vierer-Flyer A4:** Vorderseite nur **K2 Galerie Kunst & Keramik** (immer K2-Stammdaten), **ohne Termine**, mit **QR** zur Galerie-URL (`buildQrUrlWithBust`). Rückseite nur **ök2 Eingangstor** `/entdecken` mit QR. **`FlyerK2Oek2TorViererPage.tsx`**. Tests + Build grün. **Commit:** `ef91a5bd` ✅ **main** ✅
