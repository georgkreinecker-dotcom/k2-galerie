...
**Letzter Stand:** 14.03.26 – **(1) Trial 2 Wochen + AGB:** Modell „2 Wochen gratis, dann zahlen oder Daten/Lizenz verfallen“ in AGB festgehalten (AGBPage §2): Nutzer gibt Daten ein; Versicherung unverzügliche Löschung bei Nichtzahlung; AGB-Bestätigung wie üblich (WillkommenPage Checkbox). OEK2-ANMELDUNG-LIZENZIERUNG-STAND Abschnitt 5 ergänzt. **(2) Flyer & Werbedokumente – keine K2-Daten in ök2 (eisernes Gesetz):** WerbeunterlagenPage: Galerie-URL/QR abhängig von Modus (ök2 → galerieOeffentlichVorschau). FlyerK2GaleriePage: Tenant aus URL/sessionStorage; bei oeffentlich nur MUSTER_TEXTE + loadStammdaten/loadEvents('oeffentlich'). ProspektGalerieeroeffnungPage: useProspektTenant(); bei oeffentlich Stammdaten/Events/QR/Impressum nur Muster; QR → ök2-Galerie-URL. PresseEinladungK2GaleriePage: usePresseTenant(); Stammdaten aus stammdatenStorage(tenant,'gallery'), Events loadEvents(tenant). **(2) Davor:** Rechnungen/Belege ök2, mök2 Positionierung. **Nächster Schritt:** Optional: statische Präsentationsmappen-MDs auf K2-Texte prüfen; Georg testen. (Commit folgt.)
