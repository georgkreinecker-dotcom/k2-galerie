...
**Letzter Stand:** 24.03.26 – **ök2 „alt/neu“ endgültig vereinheitlicht (ein Eintritts-Standard).** Neuer zentraler Helfer **`src/utils/oek2FreshStart.ts`** setzt eine **frische Besucher-Session** (inkl. Zeitstempel). **SmartPanel** nutzt ihn für **„Als Fremder eintreten“** und jetzt auch für normale **ök2-Klicks** (führen sauber über `/entdecken`), **EntdeckenPage** nutzt denselben Helfer (`openByChoice`). **GaleriePage** akzeptiert `k2-from-entdecken` nur noch als **frische** Session (`hasFreshOek2EntrySession`) – alte/stale Flags zählen nicht mehr. Tests + Build grün.
