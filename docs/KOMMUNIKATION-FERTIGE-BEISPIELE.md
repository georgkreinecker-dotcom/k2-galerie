# Kommunikation – fertige Beispiele (redigierte Seiten)

**Stand:** 08.03.26  
**Zweck:** So sehen die Kommunikations-Dokumente aus, wenn sie **fertig ausgefüllt** sind – konkreter Text, keine Platzhalter. Zum sofortigen Lesen, Zeigen und Nutzen. Die Vorlagen (mit [Anrede] etc.) stehen in den anderen Kampagnen-Dokumenten; hier nur die **gesetzten Beispiele**.

---

## 1. Flyer – fertige Seite (Beispiel)

**Eine Seite, druckfertig – so wie sie auf den Tisch oder in die Hand kommt.**

---

**K2 Galerie – für Künstler:innen, die gesehen werden wollen.**

*Deine Ideen, deine Werke verdienen mehr als einen Instagram-Post.*

---

**Was ist K2?**

K2 Galerie ist die Plattform für Galerien, Künstler:innen und Kunstvereine: Werke präsentieren, Kasse führen, Events planen, Presse aus einer Hand. **ök2** für Künstler:innen mit eigenem Auftritt, **VK2** für Vereine – eine Oberfläche, kein Stückwerk.

---

**Demo & Links**

- **Willkommen / Demo (ök2):** https://k2-galerie.vercel.app/willkommen  
- **VK2 (Vereine):** https://k2-galerie.vercel.app/projects/vk2  

QR-Code zu einer dieser URLs aus der APf (Mission Control oder Galerie) auf den Flyer setzen – dann führt er direkt zur Demo.

---

**Kontakt**  
info@kgm.at · 0664 1046337

---

[SEITENUMBRUCH]

---

## 2. Ansprache Künstler:in – fertige E-Mail (Beispiel)

**So liest sich die Einladung an eine Künstlerin, wenn sie fertig ist.**

---

**Betreff:** Einladung – K2 Galerie / ök2 ausprobieren

Liebe Maria,

wir laden dich ein, **ök2** selbst anzuschauen – die Plattform, mit der Künstler:innen ihre Werke, Vita und Galerie im Netz präsentieren. Galerie, Kasse und Presse aus einer Hand, ohne Stückwerk.

Demo und Willkommensseite: https://k2-galerie.vercel.app/willkommen

Wenn du magst, probier es aus und sag uns, was fehlt oder was dir auffällt. Wir freuen uns auf dein Feedback.

Georg  
K2 Galerie

---

[SEITENUMBRUCH]

---

## 3. Ansprache Verein – fertige E-Mail (Beispiel)

**So liest sich die Einladung an einen Kunstverein, wenn sie fertig ist.**

---

**Betreff:** Einladung – VK2 Vereinsplattform kennenlernen

Liebe Vorstandschaft,

wir laden euch ein, **VK2** anzuschauen – die Plattform für alle Vereine (Kunstvereine = Einstieg): alle Mitglieder, alle Werke, eine gemeinsame Galerie und eine Presse-Stimme. Galerie, Kasse und Events aus einer Hand.

VK2-Demo: https://k2-galerie.vercel.app/projects/vk2

Wer mag, kann die Demo vor Ort oder von zu Hause aus ausprobieren. Wir sind auch dankbar für Verbesserungsvorschläge.

Georg  
K2 Galerie

---

[SEITENUMBRUCH]

---

## 4. E-Mail nach Lizenzkauf – fertig (Beispiel)

**So liest sich die Danke-Mail nach einem Kauf, wenn sie mit echten Links ausgefüllt ist.**

---

**Betreff:** Deine K2 Galerie – Link und nächste Schritte

Liebe Lena,

danke für deinen Lizenzkauf. Hier deine Links:

**Deine Galerie (öffentlich):** https://k2-galerie.vercel.app/g/beispiel-tenant  
**Admin (bearbeiten):** https://k2-galerie.vercel.app/admin?tenantId=beispiel-tenant

Nächster Schritt: Admin öffnen, Werke und Texte anlegen, dann „Veröffentlichen“. Deine Galerie ist dann unter der Galerie-URL sichtbar.

Bei Fragen: info@kgm.at · 0664 1046337

K2 Galerie

---

[SEITENUMBRUCH]

---

## 5. E-Mail Einladung zum Test – fertig (Beispiel)

**So liest sich die Einladung an eine Pilot:in oder Interessierte.**

---

**Betreff:** Einladung – K2 Galerie / ök2 testen

Liebe Lena,

wir laden dich ein, die K2-Demo selbst auszuprobieren – Galerie, Kasse, Presse aus einer Hand. Kein Account nötig zum Anschauen.

**Demo / Willkommen:** https://k2-galerie.vercel.app/willkommen  
**VK2 (Vereine):** https://k2-galerie.vercel.app/projects/vk2

Wenn du magst, sag uns danach kurz, was dir auffällt oder fehlt. Wir freuen uns auf dein Feedback.

Georg  
K2 Galerie

---

*Die Vorlagen mit Platzhaltern ([Anrede], [Kontakt] etc.) findest du in den anderen Kampagnen-Dokumenten (Ansprache, Flyer, E-Mail-Vorlagen). Dieses Dokument zeigt nur die fertige Form.*
