...
- **Nutzer „Profi“ (laufender Betrieb):** Admin → **Einstellungen** → Karte **Profi-Modus** – **ein Klick** blendet Fortschritts-Hinweise und Checklisten (X/4, Lesepfade, mök2-Pilot-Hinweis, Demo-Kasse) **auf diesem Gerät** aus. Gespeichert in `localStorage` (`k2-admin-hide-gamification-checklists` = `1`). Jederzeit wieder einschalten. Logik: `shouldShowGamificationChecklists()` = Schicht B per **Env** an **und** Profi-Ausblendung **nicht** gesetzt; Hook `useGamificationChecklistsUi`.
