// Patch: Ersetzt den Etiketten-Druckbutton im VK2-Kontext durch Mitgliederlisten-Druckoptionen
const fs = require('fs');
let c = fs.readFileSync('components/ScreenshotExportAdmin.tsx', 'utf8');
const lines = c.split('\n');

// Line indices (0-based): 8761 = start of the etiketten button div, 8796 = closing </div>
const startLine = 8761;
const endLine = 8796;

const before = lines.slice(0, startLine).join('\n');
const after = lines.slice(endLine + 1).join('\n');

const newBlock = `                {isVk2AdminContext() ? (
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                      <button
                        ...
                        onClick={() => {
                          ...
                          const rows = mitglieder.map((m, i) =>
                            '<tr><td>' + (i+1) + '</td><td>' + (m.name||'') + '</td><td>' + (m.typ||'') + '</td><td>' + (m.email||'') + '</td><td>' + (m.phone||'') + '</td><td>' + (m.seit||m.eintrittsdatum||'') + '</td></tr>'
                          ).join('')
                          const html = '<!DOCTYPE html><html lang="de"><head><meta charset="UTF-8"><title>Mitgliederliste</title>' +
                            '<style>body{font-family:Arial,sans-serif;padding:2cm;color:#111}h1{font-size:1.4rem;margin-bottom:0.5rem}' +
                            'table{width:100%;border-collapse:collapse;font-size:0.82rem}th{background:#333;color:#fff;padding:0.5rem 0.75rem;text-align:left}' +
                            'td{padding:0.45rem 0.75rem;border-bottom:1px solid #ddd}tr:nth-child(even) td{background:#f9f9f9}' +
                            ...
                            '<table><thead><tr><th>#</th><th>Name</th><th>Kunstbereich</th><th>E-Mail</th><th>Telefon</th><th>Mitglied seit</th></tr></thead>' +
                            '<tbody>' + rows + '</tbody></table></body></html>'
                          const w = window.open('', '_blank')
                          if (w) { w.document.write(html); w.document.close(); setTimeout(() => w.print(), 400) }
                        }}
                        style={{ padding: '0.7rem 1.2rem', background: s.bgElevated, color: s.text, border: \`1px solid \${s.accent}44\`, borderRadius: '10px', fontSize: '0.88rem', fontWeight: 600, cursor: 'pointer' }}
                      >
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          const karten = mitglieder.map(m => {
                            const adresse = [m.strasse, (m.plz && m.ort) ? (m.plz + ' ' + m.ort) : (m.ort || '')].filter(Boolean).join(', ')
                            return '<div class="karte">' +
                              '<div class="name">' + (m.name||'') + '</div>' +
                              (adresse ? '<div class="detail">' + adresse + '</div>' : '') +
                              ((m.email || m.phone) ? '<div class="detail">' + (m.email||'') + (m.phone ? ' | ' + m.phone : '') + '</div>' : '') +
                              '<div class="detail">' + (m.typ||'') + (m.seit||m.eintrittsdatum ? ' | Mitglied seit ' + (m.seit||m.eintrittsdatum) : '') + '</div>' +
                              '</div>'
                          }).join('')
                          const html = '<!DOCTYPE html><html lang="de"><head><meta charset="UTF-8"><title>Adressliste</title>' +
                            '<style>body{font-family:Arial,sans-serif;padding:2cm;color:#111}h1{font-size:1.4rem;margin-bottom:0.5rem}' +
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          a.href = url; a.download = 'mitglieder-' + new Date().toISOString().slice(0,10) + '.csv'
...
fs.writeFileSync('components/ScreenshotExportAdmin.tsx', newContent, 'utf8');
