// Patch: Etiketten-Button-Block durch VK2-bewusste Version ersetzen
// Lines 8762-8796 (1-indexed), 0-indexed: 8761-8795
const fs = require('fs');
let c = fs.readFileSync('components/ScreenshotExportAdmin.tsx', 'utf8');
const lines = c.split('\n');

const startIdx = 8761; // 0-indexed
const endIdx = 8796;   // 0-indexed, exclusive (line after closing </div>)

// Verify we're at the right spot
if (!lines[8761].includes("display: 'flex', flexDirection: 'column', gap: '0.3rem'")) {
  console.error('WRONG POSITION! Found:', lines[8761].slice(0, 80));
  process.exit(1);
}

const PRINT_BUTTON_STYLE = "{ padding: '0.7rem 1.2rem', background: s.bgElevated, color: s.text, border: `1px solid ${s.accent}44`, borderRadius: '10px', fontSize: '0.88rem', fontWeight: 600, cursor: 'pointer' }";
...
const newLines = [
  ...
  "                            return '<div class=\"karte\"><div class=\"name\">' + (m.name||'') + '</div>' +",
  "                              (adresse ? '<div class=\"detail\">' + adresse + '</div>' : '') +",
  "                              ((m.email || m.phone) ? '<div class=\"detail\">' + (m.email||'') + (m.phone ? ' | ' + m.phone : '') + '</div>' : '') +",
  "                              '<div class=\"detail\">' + (m.typ||'') + (m.seit||m.eintrittsdatum ? ' | Mitglied seit ' + (m.seit||m.eintrittsdatum) : '') + '</div></div>'",
  "                          }).join('')",
