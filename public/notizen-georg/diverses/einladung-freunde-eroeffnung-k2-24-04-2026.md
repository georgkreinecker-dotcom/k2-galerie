...
- **Links:** **Echte K2-Galerie mit Werken** = `/galerie` (nicht `/willkommen` – das ist nur die Willkommensseite). **ök2-Besucher-Eintritt** = Basis-URL `/` (leitet auf **Entdecken**); explizit auch `/entdecken`. **Präsentationsmappe Langform** = `…/praesentationsmappe-vollversion`; Kurzform eine Seite = `…/praesentationsmappe`.  
